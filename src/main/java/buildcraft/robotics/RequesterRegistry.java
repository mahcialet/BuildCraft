...
public final class RequesterRegistry {
    ...
    public static Optional<Reservation> reserveClosest(ServerLevel level, Vec3 origin, UUID robot,
        ...
        return data.requesters.entrySet().stream()
                ...
                .filter(candidate -> Vec3.atCenterOf(candidate.position).distanceToSqr(origin)
                ...
                .map(candidate -> {
                    Key key = new Key(candidate.position, candidate.slot);
                    data.reservations.put(key, robot);
                    return new Reservation(candidate.position, candidate.slot, candidate.request.copy(), robot);
                });
    }
    ...
    public static boolean reclaim(ServerLevel level, Reservation reservation) {
        ...
        UUID owner = data.reservations.get(new Key(reservation.position, reservation.slot));
        ...
        data.reservations.put(new Key(reservation.position, reservation.slot), reservation.robot);
        return true;
    }
    ...
    public static void release(ServerLevel level, Reservation reservation) {
        LevelData data = LEVELS.get(level);
        if (data == null) return;
        data.reservations.remove(new Key(reservation.position, reservation.slot), reservation.robot);
    }
